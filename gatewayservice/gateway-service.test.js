...
describe('Gateway Service', () => {
  beforeEach(() => {
    ...
    axios.post.mockImplementation((url, data) => {
      if (url.endsWith('/login')) {
        ...
        return Promise.resolve({ data: { answer: 'AI generó esta respuesta' } });
      } else if (url.endsWith('/generate-apikey')) {
        return Promise.resolve({ data: { apiKey: 'mock-api-key-12345' } });
      } else if (url.endsWith('/savestats')) {
        return Promise.resolve({ data: { success: true, message: 'Estadísticas guardadas' } });
      }
    });
    ...
    axios.get.mockImplementation((url) => {
      if (url.includes('/validate-apikey/')) {
        ...
        const response = url.includes('/questionsDB')
            ? { data: [{ id: 1, question: '¿Pregunta de la base de datos?', answer: 'respuesta', topic: 'historia' }] }
            : { data: [{ id: 1, question: '¿Pregunta generada?', answer: 'respuesta', topic: 'ciencia' }] };
  ...
  it('should forward add user request to user service', async () => {
    ...
    expect(axios.post).toHaveBeenCalledWith(expect.stringContaining('/adduser'),
  ...
  it('should forward ai-answer request to the llm service', async () => {
    ...
    expect(axios.post).toHaveBeenCalledWith(expect.stringContaining('/ai-answer'),
  ...
  it('should forward savestats request to the stats service', async () => {
    ...
    expect(axios.post).toHaveBeenCalledWith(expect.stringContaining('/savestats'), statsData);
  });
  ...
  it('should handle savestats service errors with request failure', async () => {
    ...
    expect(response.statusCode).toBe(500);
    expect(response.body.error).toBe('El servidor de estadísticas no está disponible');
  });
  ...
  it('should forward getstats request to the stats service', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(expect.stringContaining(`/getstats/${username}`));
  });
  ...
  it('should forward games request to the stats service', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(expect.stringContaining(`/games/${username}`));
  });
  ...
  it('should forward ratios-per-month request to the stats service', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(expect.stringContaining(`/ratios-per-month/${username}`));
  });
  ...
  it('should handle ratios-per-month service errors', async () => {
    ...
    const response = await request(app).get('/ratios-per-month/nonexistentuser');
  ...
  it('should forward questions request to the wiki question service', async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual([
      { id: 1, question: '¿Pregunta generada?', answer: 'respuesta', topic: 'ciencia' }
    ]);
    expect(axios.get).toHaveBeenCalledWith(expect.stringContaining('/questions?n=5&topic=ciencia'));
  });
  ...
  it('should use default values for questions request if not provided', async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(axios.get).toHaveBeenCalledWith(expect.stringContaining('/questions?n=10&topic=all'));
  });
  ...
  it('should forward questionsDB request to the wiki question service', async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(response.body).toEqual([
      { id: 1, question: '¿Pregunta de la base de datos?', answer: 'respuesta', topic: 'historia' }
    ]);
    expect(axios.get).toHaveBeenCalledWith(expect.stringContaining('/questionsDB?n=5&topic=historia'));
  });
  ...
  it('should use default values for questionsDB request if not provided', async () => {
    ...
    expect(response.statusCode).toBe(200);
    expect(axios.get).toHaveBeenCalledWith(expect.stringContaining('/questionsDB?n=10&topic=all'));
  });
  ...
  it('should forward generate-apikey request to the user service', async () => {
    ...
    expect(axios.post).toHaveBeenCalledWith(expect.stringContaining('/generate-apikey'),
        { username: 'testuser' });
  });

  it('should handle errors from generate-apikey endpoint', async () => {
    axios.post.mockImplementationOnce((url) => {
  ...
  it('should forward validate-apikey request to the user service', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(expect.stringContaining(`/validate-apikey/${apiKey}`));
  });
