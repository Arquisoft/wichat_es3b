...
|Game logic is handled on the frontend instead of the backend|Ideally, the game logic should reside on the backend to improve security and maintainability. However, due to time constraints, it remains on the frontend for now. This refactor is planned for future iterations.
|===
